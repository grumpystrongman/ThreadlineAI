...
public sealed class SqliteThreadlineStore : ISessionRepository, IProviderConnectionRepository, IAuditRepository, IThreadlineStoreInitializer
{
    ...
    public async Task SaveSessionAsync(ThreadlineSession session, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$createdAtUtc", SqliteHelpers.ToText(session.CreatedAt));
        command.Parameters.AddWithValue("$status", session.Status.ToString());
        command.Parameters.AddWithValue("$activeProvider", SqliteHelpers.ToDbValue(session.ActiveProvider));
        command.Parameters.AddWithValue("$endedAtUtc", SqliteHelpers.ToNullableText(session.EndedAt));
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    public async Task AppendEventAsync(ContextEvent contextEvent, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$timestampUtc", SqliteHelpers.ToText(contextEvent.Timestamp));
        ...
        command.Parameters.AddWithValue("$applicationName", SqliteHelpers.ToDbValue(contextEvent.ApplicationName));
        command.Parameters.AddWithValue("$processName", SqliteHelpers.ToDbValue(contextEvent.ProcessName));
        command.Parameters.AddWithValue("$windowTitle", SqliteHelpers.ToDbValue(contextEvent.WindowTitle));
        command.Parameters.AddWithValue("$uri", SqliteHelpers.ToDbValue(contextEvent.Uri));
        ...
        command.Parameters.AddWithValue("$metadataJson", SqliteHelpers.ToJsonDbValue(contextEvent.Metadata));
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    public async Task SaveSummaryAsync(string sessionId, string summary, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$sessionId", sessionId);
        command.Parameters.AddWithValue("$createdAtUtc", SqliteHelpers.ToText(DateTimeOffset.UtcNow));
    ...
    public async Task SaveProviderConnectionAsync(ProviderConnection connectionRecord, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$credentialReference", SqliteHelpers.ToDbValue(connectionRecord.CredentialReference));
        command.Parameters.AddWithValue("$baseUrl", SqliteHelpers.ToDbValue(connectionRecord.BaseUrl));
        command.Parameters.AddWithValue("$defaultModel", SqliteHelpers.ToDbValue(connectionRecord.DefaultModel));
        command.Parameters.AddWithValue("$status", connectionRecord.Status.ToString());
        command.Parameters.AddWithValue("$createdAtUtc", SqliteHelpers.ToText(connectionRecord.CreatedAt));
        command.Parameters.AddWithValue("$updatedAtUtc", SqliteHelpers.ToNullableText(connectionRecord.UpdatedAt));
        command.Parameters.AddWithValue("$metadataJson", SqliteHelpers.ToJsonDbValue(connectionRecord.Metadata));
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    public async Task AppendAuditEventAsync(AuditEvent auditEvent, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("$sessionId", SqliteHelpers.ToDbValue(auditEvent.SessionId));
        command.Parameters.AddWithValue("$eventType", auditEvent.EventType.ToString());
        command.Parameters.AddWithValue("$timestampUtc", SqliteHelpers.ToText(auditEvent.Timestamp));
        command.Parameters.AddWithValue("$message", auditEvent.Message);
        command.Parameters.AddWithValue("$metadataJson", SqliteHelpers.ToJsonDbValue(auditEvent.Metadata));
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    private static ThreadlineSession ReadSession(SqliteDataReader reader) => new(
        reader.GetString(0),
        reader.GetString(1),
        SqliteHelpers.FromText(reader.GetString(2)),
        Enum.Parse<SessionStatus>(reader.GetString(3)),
        reader.IsDBNull(4) ? null : reader.GetString(4),
        reader.IsDBNull(5) ? null : SqliteHelpers.FromText(reader.GetString(5)));
    ...
    private static ContextEvent ReadContextEvent(SqliteDataReader reader) => new(
        ...
        SqliteHelpers.FromJson(reader.IsDBNull(12) ? null : reader.GetString(12)));
    ...
    private static ProviderConnection ReadProviderConnection(SqliteDataReader reader) => new(
        ...
        SqliteHelpers.FromText(reader.GetString(7)),
        reader.IsDBNull(8) ? null : SqliteHelpers.FromText(reader.GetString(8)),
        SqliteHelpers.FromJson(reader.IsDBNull(9) ? null : reader.GetString(9)));

    private static AuditEvent ReadAuditEvent(SqliteDataReader reader) => new(
        reader.GetString(0),
        reader.IsDBNull(1) ? null : reader.GetString(1),
        Enum.Parse<AuditEventType>(reader.GetString(2)),
        SqliteHelpers.FromText(reader.GetString(3)),
        reader.GetString(4),
        SqliteHelpers.FromJson(reader.IsDBNull(5) ? null : reader.GetString(5)));
